...
from synembtrack.EmbedSeg.criterions import get_loss
from synembtrack.EmbedSeg.datasets import get_dataset
from synembtrack.EmbedSeg.models import get_model
from synembtrack.EmbedSeg.utils.utils import AverageMeter, Cluster, Cluster_3d, Logger, Visualizer, prepare_embedding_for_train_image
...
#        if i >600:
 #           pass
        ...
        instances = sample['instance'].squeeze(1).cuda()
        class_labels = sample['label'].squeeze(1).cuda()
        center_images = sample['center_image'].squeeze(1).cuda()
...
def train_vanilla(display, display_embedding, display_it, one_hot, grid_x, grid_y, pixel_x, pixel_y, n_sigma,
    ...
    for i, sample in enumerate(tqdm(train_dataset_it)):
        ...
        im = sample['image'].cuda()
        instances = sample['instance'].squeeze(1).cuda()  # 1YX (not one-hot) or 1DYX (one-hot)
        class_labels = sample['label'].squeeze(1).cuda()  # 1YX
        center_images = sample['center_image'].squeeze(1).cuda()  # 1YX
        ...
        argument 분석
        im: Batch,      Batch, -, Y, X
        instances:      Batch, I, Y, X
        labels:         1/0,      Y, X
        center_images:  1/0,      Y, X  (position of centers of each instance)
        output:         Batch, five outputs, Y, X
        output1 = offset1 ?
        output2 = offset2 ?
        output3 = bandwidth1 ?
        output4 = bandwidth2 ?
        ...
        '''
        #print("이곳을 보아라")
        #print(im.shape, instances.shape, class_labels.shape, center_images.shape, output.shape)
        #print(sum(sum(sum(labels))))
        
        #print(im)
        #print('################################################################################')
        #print(output.shape, instances.shape, class_labels.shape, center_images.shape)
        #print('################################################################################')
        ...
        if display and i % display_it == 0:
            with torch.no_grad():
                visualizer.display(im[0].cpu(), key='image', title='Image')
...
def save_checkpoint(state, is_best, epoch, save_dir, save_checkpoint_frequency, name='checkpoint.pth'):
    ...
    name = f'checkpoint_epoch{epoch:03d}.pth'
    ...
    if is_best:
        shutil.copyfile(file_name, os.path.join(
            save_dir, f'best_iou_model_{epoch:03d}.pth'))
        shutil.copyfile(file_name, os.path.join(
            save_dir, f'best_iou_model.pth'))
...
def begin_training(train_dataset_dict, val_dataset_dict, model_dict, loss_dict, configs, color_map='magma'):
    ...
    # Learning
    for epoch in range(start_epoch, configs['n_epochs']+1):
