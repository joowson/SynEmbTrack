...
def split_train_test(data_dir, project_name, train_test_name, subset=0.5, by_fraction=True, seed=1000):
    ...
    image_names = sorted(glob(os.path.join(image_dir, '*.tif')))
    #print('path:', os.path.join(image_dir, '*.tif'))
    #print(len(image_names), 'exmaple', image_names[0])
...
def calculate_object_size(data_dir, project_name, train_val_name, mode, one_hot, process_k):
    ...
    if mode =='3d':
        ...
        return np.min(size_list).astype(np.float64), np.mean(size_list_z).astype(np.float64), np.mean(size_list_y).astype(np.float64), np.mean(size_list_x).astype(np.float64), \
               np.std(size_list_z).astype(np.float64), np.std(size_list_y).astype(np.float64), np.std(size_list_x).astype(np.float64)

    else:
        return np.min(size_list).astype(np.float64), None, np.mean(size_list_y).astype(
            np.float64), np.mean(size_list_x).astype(np.float64), None, np.std(size_list_y).astype(
            np.float64), np.std(size_list_x).astype(np.float64)
...
def calculate_max_eval_image_size(data_dir, project_name, test_name, mode, one_hot):
    ...
    #print(instance_dir)
    #print(os.path.join(instance_dir, '*.tif'))
    #print(image_names)
    ...
    if mode == '2d':
        ...
        if (max_y % 8 != 0):
            n = max_y // 8
            #print(n,max_y)
            max_y = np.clip((n + 1) * 8, a_min=1920, a_max=None)
        else:
            max_y = np.clip(max_y, a_min=1920, a_max=None)
        if (max_x % 8 != 0):
            n = max_x // 8
            max_x = np.clip((n + 1) * 8, a_min=1920, a_max=None)
        else:
            max_x = np.clip(max_x, a_min=1920, a_max=None)
        ...
        return None, max_y.astype(np.float64), max_x.astype(np.float64)
    elif mode == '3d':
        ...
        return max_z.astype(np.float64), max_y.astype(np.float64), max_x.astype(np.float64)
