...
from synembtrack.EmbedSeg.datasets import get_dataset
from synembtrack.EmbedSeg.models import get_model
from synembtrack.EmbedSeg.utils.utils import Cluster, Cluster_3d, prepare_embedding_for_test_image
from synembtrack.EmbedSeg.utils.utils2 import matching_dataset, obtain_AP_one_hot
...
from synembtrack.EmbedSeg.IntegerMask2ContoursDraw import draw_mask_contour
...
def begin_evaluating(test_configs, verbose=True, mask_region = None, mask_intensity = None, avg_bg = None):
    ...
    global tta, seed_thresh, model, dataset_it, save_images, save_results, save_dir, data_dir
    ...
    data_dir = test_configs['dataset']['kwargs']['data_dir']
...
def test(verbose, grid_y=1024, grid_x=1024, pixel_y=1, pixel_x=1, one_hot = False, avg_bg = 0, n_sigma = 2):
    ...
    with torch.no_grad():
        ...
        for sample in tqdm(dataset_it):
            ...
            '''
            print(type(im), im.shape,max(im))
            print((im[0].numpy().transpose(1, 2, 0)*100).max())
            cv2.imshow('f', im[0].numpy().transpose(1, 2, 0))
            cv2.waitKey(0)
            ...
            ## pad
            pad_ele_1, pad_ele_2, pad_ele_3, pad_ele_4 = diff_x // 2, diff_x - diff_x // 2, diff_y // 2, diff_y - diff_y // 2
            p2d = (pad_ele_1, pad_ele_2, pad_ele_3, pad_ele_4)  # last dim, second last dim
            im = F.pad(im, p2d, "constant", avg_bg)  # padding
            ...
            ## debugging ---for checking progress
            for i in range(5):

                pass
                #vis = output[0].cpu().numpy().transpose(1, 2, 0)[:,:,i]
                #print(vis.max())
                '''
                cv2.imshow(f'{i}.png', vis*100)
                cv2.waitKey(0)
                cv2.destroyAllWindows()
                cv2.imwrite(f'{i}.png', vis*100)
                '''
            ...
            if save_images:# and ap_val == 0.5:
                ...
                # if not os.path.exists(os.path.join(save_dir, 'result_visualize/')):
                #     os.makedirs(os.path.join(save_dir, 'result_visualize/'))
                #     print("Created new directory {}".format(os.path.join(save_dir, 'result_visualize/')))

                ## save name 0f predictions
                base, _ = os.path.splitext(os.path.basename(sample['im_name'][0]))
                instances_file = os.path.join(save_dir, 'predictions/', base + '.tif')
                frame_num = base[-4:]

                ## padding으로 변형된 사이즈 원상복귀
                if       (pad_ele_3 == 0 and pad_ele_4 == 0) and     (pad_ele_1 == 0 and pad_ele_2 == 0):
                    pass

                elif     (pad_ele_3 == 0 and pad_ele_4 == 0) and not (pad_ele_1 == 0 and pad_ele_2 == 0):
                    instance_map = instance_map[:, pad_ele_1:-pad_ele_2]

                elif not (pad_ele_3 == 0 and pad_ele_4 == 0) and     (pad_ele_1 == 0 and pad_ele_2 == 0):
                    instance_map = instance_map[pad_ele_3:-pad_ele_4, :]

                elif not (pad_ele_3 == 0 and pad_ele_4 == 0) and not (pad_ele_1 == 0 and pad_ele_2 == 0):
                    instance_map = instance_map[pad_ele_3:-pad_ele_4, pad_ele_1:-pad_ele_2]
                ...
                img = Image.fromarray(instance_map.cpu().detach().numpy().astype(np.uint16))
                ...
                ## draw contour
                # if int(frame_num) <100 or np.mod(int(frame_num), 10) ==0:
                if True:
                    draw_mask_contour(data_dir, save_dir / 'predictions', save_dir / 'frms_with_cnts', frame_num = frame_num)
...
#                 cv2.imwrite(embedding_file, arr)
#
#                 org_file = os.path.join(save_dir, 'embedding/', base + '_org.tif')
#                 #cv2.imwrite(org_file, im[0,0,:,:].numpy()*255)
#                 #print(im[0,0,:,:].numpy()*255, im[0,0,:,:].numpy().max()*255)
#
